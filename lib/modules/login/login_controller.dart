...
class LoginController extends GetxController with LoaderMixin, MessagesMixin {
  ...
  Future<void> loginWithEmail() async {
    try {
      loading(true);
      //var user = await _loginService.loginWithEmail();
      loading(false);
      message(MessageModel(title: 'Sucesso', message: 'Sucesso ao efetuar login com email', type: MessageType.info));
      await Get.offAllNamed('/home', parameters: {'type': '${SOCIALLOGINTYPE.email}'});
    } catch (_) {
      Get.back();
  ...
  Future<void> loginWithFacebook() async {
    try {
      ...
      await Get.offAllNamed("/home", arguments: user, parameters: {'type': '${SOCIALLOGINTYPE.facebok}'});
    } catch (_) {
      Get.back();
      message(MessageModel.error(title: 'Falhou', message: 'Falha ao efetuar login com facebook'));
    }
